...
def main(argv: list[str] | None = None) -> int:
    ...
    if operation == "inspect":
        if len(arguments) != 2:
            return argument_failure(operation, "Usage: loopx graph inspect <task-dir>")
        payload, exit_code = inspect(Path(arguments[1]).expanduser().resolve())
    elif operation == "show":
        if len(arguments) != 3:
            return argument_failure(operation, "Usage: loopx graph show <task-dir> <ticket-id>")
        payload, exit_code = show_ticket(Path(arguments[1]).expanduser().resolve(), arguments[2])
    elif operation == "list":
        if len(arguments) < 2 or (len(arguments) - 2) % 2:
            return argument_failure(operation, "Usage: loopx graph list <task-dir> [--phase <phase>] [--readiness <ready|blocked>]")
        phase: str | None = None
        ...
        if len(arguments) != 5 or arguments[3] != "--input": return argument_failure(operation, f"Usage: loopx graph {operation} <task-dir> <ticket-id> --input <path|->")
        ...
        if len(arguments) != 4 or arguments[2] != "--input": return argument_failure(operation, f"Usage: loopx graph {operation} <task-dir> --input <path|->")
