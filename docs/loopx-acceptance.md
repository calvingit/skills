...
| 已有新进度；之后某个窗口没有新进度 | 先继续等待，之后满足条件时仍可使用唯一一次询问机会。 |
| 多次等待、状态恢复明确后再次未知、恢复上下文 | 读取原执行的询问记录，不重复发送；发送结果不确定也不重发。 |
| 询问后无回复；工具调用结束后才收到回复 | 留出正常响应机会；记录未收到或迟到回复，不否定已知 running，不关闭、不重派。 |
| Runtime 不支持运行中消息 | 记录限制并继续等待，不以中断代替询问，不增加验收前置条件。 |
| 达到 600 秒无活动阈值 | 核实状态和已知运行命令，再有序恢复；新活动可更新阈值，明确总预算不续期，不增加 pong 超时。 |
...
| 旧 running 状态与较新的 Runtime 停止确认冲突 | 使用同一执行的较新明确事实；Agent 自述不能代替停止确认。 |
| ticket 本地检查通过；最终独立报告缺失 | ticket 可完成并放行依赖，最终交付不可批准；额外明确要求的 ticket 独立检查仍须满足。 |
