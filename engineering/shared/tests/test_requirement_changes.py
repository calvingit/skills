...
from ticket_graph.queries import inspect
from planning import reconcile_batch
from ticket_graph.authority import authority_fingerprint
import delivery
...
class RequirementChangesTests(unittest.TestCase):
    ...
    def test_new_acceptance_can_stop_old_attempt_before_reconciliation(self):
        ...
        extra = {key: value for key, value in ticket().items() if key in {'title','covers','referenced_design_decisions','what_to_build','constraints','delivery_acceptance','dependencies'}}
        extra['covers'] = {'requirements': [], 'spec_acceptance': ['AC2']}
        extra['delivery_acceptance'] = [{'id': 'AC1', 'description': 'Additional behavior.'}]
