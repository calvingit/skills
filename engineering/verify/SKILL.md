...
4. Inspect changed tests, fixtures, snapshots, mocks and verification configuration only as far as evidence credibility requires. Weakened, skipped, implementation-derived or otherwise invalidated checks are insufficient even when green. State mocked boundaries, bypassed paths, platform limits and visual/UX gaps; behavioural success does not establish visual or interaction quality. Broader maintainability and regression review belong to `code-review`.
