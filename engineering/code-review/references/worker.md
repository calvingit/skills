...
必需：`review_mode`、`review_scope`、`user_request`、当前 ticket 的 `R`/`AC` 和验收协议。可选 `review_target`，默认 `diff`。
...
5. Contract worker 接收 `SPEC.md`、`ACCEPTANCE.md` 和失败状态矩阵；Change-surface worker 接收直接调用链、公开类型、测试和配置；Exploratory worker 可以扩展观察范围，但必须把范围外问题标记为 `out_of_scope_risk`。按 [review-criteria.md](review-criteria.md) 检查，并返回 [output-contract.md](output-contract.md) 规定的回执。
