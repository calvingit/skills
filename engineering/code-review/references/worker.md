...
5. Standards worker 同时接收项目 standards 与 [smell-baseline.md](smell-baseline.md) 全文；Spec worker 接收实际需求来源；HLD worker 接收完整 HLD、当前 ticket 的 D 引用和局部实现空间。返回 `references/output-and-rules.md` 规定的 Markdown 回执。
