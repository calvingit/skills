...
3. worker 只处理分配给自己的一个轴。没有独立 reviewer 时，当前 Agent 才前后分开审查 Standards 与 Spec。
4. 只报告有证据证明由本次变更引入或扩大的问题。
5. Standards worker 同时接收项目 standards 与 [smell-baseline.md](smell-baseline.md) 全文；Spec worker 接收实际需求来源。返回 `references/output-and-rules.md` 规定的 Markdown receipt。
