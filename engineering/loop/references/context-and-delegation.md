...
Give each delegated Agent its bounded question or ticket, current attempt when applicable, relevant context notes and authoritative requirements, baseline, existing edits, expected change areas, and required evidence. Include the resolved project verification entry, affected surfaces, permitted test resources and evidence location; let verify independently select sufficient checks from those sources. Read relevant notes first, then inspect current files. Return a concise report under the task's `.loop/` or through native results; exploratory conclusions do not silently amend requirements or shared design. Do not create a permanent Agent pool or dispatch one Agent for every ticket.
