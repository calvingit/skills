...
Profile 只保存稳定的模式和项目内说明入口，不保存当前 PRD 内容、临时链接或某次任务的需求快照。`to-spec` 是主要消费者；`grilling` 和 `wayfinding` 只用它判断哪些需求事实必须由用户提供。下游 `to-tickets`、`loop`、`implement` 和 `code-review` 只消费已确认的 SPEC 或 tickets，不直接解释该配置。
