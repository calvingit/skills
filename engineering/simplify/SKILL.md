...
7. 修改后至少执行残留检查、能够发现误删的最小必要验证，以及差异审查。若改变需求、公开约定或验收，停止并交回 `grilling` / `to-spec`；若只改变多处实现共用的职责归属、共享设计约束、依赖方向或集成约束，停止并交回 `high-level-design`。上游产物修订后，再由 `quick-implement` 或 `loop` 按原执行范围继续。
